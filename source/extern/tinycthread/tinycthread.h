...
* not be honored.
*/
ENGINE_API int mtx_init(mtx_t *mtx, int type);

/** Release any resources used by the given mutex.
* @param mtx A mutex object.
*/
ENGINE_API void mtx_destroy(mtx_t *mtx);
...
* not be honored.
*/
ENGINE_API int mtx_lock(mtx_t *mtx);
...
*/
ENGINE_API int mtx_timedlock(mtx_t *mtx, const struct timespec *ts);
...
* honored.
*/
ENGINE_API int mtx_trylock(mtx_t *mtx);
...
* not be honored.
*/
ENGINE_API int mtx_unlock(mtx_t *mtx);
...
* not be honored.
*/
ENGINE_API int cnd_init(cnd_t *cond);

/** Release any resources used by the given condition variable.
* @param cond A condition variable object.
*/
ENGINE_API void cnd_destroy(cnd_t *cond);
...
* not be honored.
*/
ENGINE_API int cnd_signal(cnd_t *cond);
...
* not be honored.
*/
ENGINE_API int cnd_broadcast(cnd_t *cond);
...
* not be honored.
*/
ENGINE_API int cnd_wait(cnd_t *cond, mtx_t *mtx);
...
*/
ENGINE_API int cnd_timedwait(cnd_t *cond, mtx_t *mtx, const struct timespec *ts);
...
* thread.
*/
ENGINE_API int thrd_create(thrd_t *thr, thrd_start_t func, void *arg);

/** Identify the calling thread.
* @return The identifier of the calling thread.
*/
ENGINE_API thrd_t thrd_current(void);

/** Dispose of any resources allocated to the thread when that thread exits.
 * @return thrd_success, or thrd_error on error
*/
ENGINE_API int thrd_detach(thrd_t thr);
...
*/
ENGINE_API int thrd_equal(thrd_t thr0, thrd_t thr1);

/** Terminate execution of the calling thread.
* @param res Result code of the calling thread.
*/
ENGINE_API TTHREAD_NORETURN void thrd_exit(int res);
...
* not be honored.
*/
ENGINE_API int thrd_join(thrd_t thr, int *res);
...
*/
ENGINE_API int thrd_sleep(const struct timespec *duration, struct timespec *remaining);

/** Yield execution to another thread.
* Permit other threads to run, even if the current thread would ordinarily
* continue to run.
*/
ENGINE_API void thrd_yield(void);
...
*/
ENGINE_API int tss_create(tss_t *key, tss_dtor_t dtor);
...
*/
ENGINE_API void tss_delete(tss_t key);
...
* storage.
*/
ENGINE_API void *tss_get(tss_t key);
...
* not be honored.
*/
ENGINE_API int tss_set(tss_t key, void *val);
 ...
 */
#if defined(_TTHREAD_WIN32_)
  ENGINE_API void call_once(once_flag *flag, void (*func)(void));
#else
