...
namespace mod {
  ...
  struct RenderMesh3D {
    ...
    /* Disable uv coordinates for a RenderMesh3D and clean up its data.
    ...
    /* Disable colors for a RenderMesh3D and clean up its data.
    ...
    ENGINE_API VertexRef3D get_vertex (size_t index) const;
    ...
    ENGINE_API void set_vertex (size_t index, VertexData3D const& data);
    ...
    ENGINE_API void append_vertex (VertexData3D const& data);


    /* Insert the various attributes of a vertex in a RenderMesh3D at a given index.
     * If a particular attribute is disabled on the mesh, the input data should be disabled as well.
     * Panics if unexpected attributes were supplied, or if attributes were expected but not supplied */
    ENGINE_API void insert_vertex (size_t index, VertexData3D const& data);
